...
public:
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        for (int c = 0; c < cols; c++){
            dfs(heights, pacificMatrix, 0, c, heights[0][c]);
        }

        for (int r = 0; r < rows; r++){
            dfs(heights, pacificMatrix, r, 0, heights[r][0]);
        }

        // Atlantic Ocean Loops for its Borders (bottom and right)
        for (int c = 0; c < cols; c++){
            dfs(heights, atlanticMatrix, rows - 1, c, heights[rows - 1][c]);
        }

        for (int r = 0; r < rows; r++){
            dfs(heights, atlanticMatrix, r, cols - 1, heights[r][cols - 1]);
        }
